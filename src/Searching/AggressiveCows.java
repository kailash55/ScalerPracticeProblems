...
public class AggressiveCows {
	...
	public static void main(String[] args) {
		...
		System.out.println(new AggressiveCowsSolution().solve(A, 385));
	}
...
//		}
		...
		int start = 0;
		int end = (int)A[A.length-1];
		
		int maxLength = -1;
		while(start <= end)
		{
			int mid = (start + end) / 2;
			boolean isPossible = checkIfArrangementIsPossible(A, mid, B);
	...
	private boolean checkIfArrangementIsPossible(int[] a, int d, int numberOfCows) {
		// TODO Auto-generated method stub
		
		int lastCowPlaced = 0;
		for(int i=0; i<a.length; i++)
		{
			if((a[i] - lastCowPlaced >= d) || i==0)
			{
				numberOfCows--;
				lastCowPlaced = (int)a[i];
			}
		}
